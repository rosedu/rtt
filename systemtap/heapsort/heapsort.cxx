...
list_type input_data()
{
  ...
  TRACE(HEAPSORT_INPUT_START());   // SystemTap trace start reading in data
  data.count=0;
  ...
  while (success==1){
    ...
    if (data.count >= capacity){
      TRACE(HEAPSORT_BUFFER_RESIZE_START()); // SystemTap trace resizing 
      // make bigger buffer
	...
	temp_buffer[i] = data.entry[i];
      ...
      capacity = capacity*2;
      TRACE(HEAPSORT_BUFFER_RESIZE_DONE()); // SystemTap trace finish resizing 
    }
  ...
  TRACE(HEAPSORT_INPUT_DONE(data.count)); // SystemTap trace finish reading in data
...
void output_data(list_type data)
{
  ...
  TRACE(HEAPSORT_OUTPUT_START(data.count)); // Systemtap trace writing sorted data
  // print out each of the sorted elements
  for (i=0; i<data.count; ++i){
    printf("%d\n", data.entry[i].key);
  }
  TRACE(HEAPSORT_OUTPUT_DONE()); // SystemTap trace finish writing sorted data
}
...
void insert_heap(list_type *lp, item_type item, int start, int maxheap)
{
        ...
        while (m<=maxheap){
			...
			TRACE(HEAPSORT_HEAP_PLACE(m,item.key));
...
{
        int i;
	TRACE(HEAPSORT_HEAP_BUILD_START()); // SystemTap start building heap

        for (i=(lp->count)-1; i>=0; --i){
                insert_heap(lp, lp->entry[i], i, lp->count-1);
        }
	TRACE(HEAPSORT_HEAP_BUILD_DONE()); // SystemTap finished building heap
}
